...
class Repository(Base):
    ...
    # Public ranking is opt-in.  Keep this in the registry row so a ranking
    # query never has to inspect repository-local files or raw health facts.
...
class RepositoryHealthSnapshot(Base):
    """Append-only canonical health envelope for one replayable source run."""
    ...
    analyzer_versions_digest: Mapped[str] = mapped_column(String(64), nullable=False, default="")
    score_config_digest: Mapped[str] = mapped_column(String(64), nullable=False, default="")
    ...
    status: Mapped[str] = mapped_column(String(24), nullable=False, default="inconclusive")
    score: Mapped[float | None] = mapped_column(Float, nullable=True)
    confidence: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    unknown_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    error_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    skipped_weight: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    evidence_coverage: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    criticality: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    stale_after_ts: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "as_of_ts",
            "config_digest",
            "analyzer_versions_digest",
            "scope",
            name="uq_repository_health_snapshot_replay",
        ),
        Index("ix_repository_health_snapshots_repo_asof", "repository_id", "as_of_ts"),
        Index("ix_repository_health_snapshots_repo_status", "repository_id", "status"),
    )
...
class HealthScoreProjection(Base):
    ...
    __tablename__ = "health_score_projections"

    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=_new_uuid)
    snapshot_id: Mapped[str] = mapped_column(
        String(32), ForeignKey("repository_health_snapshots.id", ondelete="CASCADE"), nullable=False
    )
    score_config_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    ...
    configured_weight: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    available_weight: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    confidence: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    ...
    score_recomputed: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now_utc)

    __table_args__ = (
        UniqueConstraint("snapshot_id", "score_config_digest", name="uq_health_score_projection_config"),
        Index("ix_health_score_projections_snapshot", "snapshot_id", "created_at"),
    )


class RepositoryHealthRankingEntry(Base):
    """Public-safe materialized headline for the repository ranking."""

    __tablename__ = "repository_health_ranking"
    ...
    __table_args__ = (
        UniqueConstraint("repository_id", name="uq_repository_health_ranking_repo"),
        Index("ix_repository_health_ranking_public", "eligible", "overall_score"),
    )


class HealthSourceRun(Base):
    """One analyzer/source execution recorded before derived rows are written."""

    __tablename__ = "health_source_runs"

    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=_new_uuid)
    snapshot_id: Mapped[str] = mapped_column(
        String(32), ForeignKey("repository_health_snapshots.id", ondelete="RESTRICT"), nullable=False
    )
    ...
    raw_fact_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    diagnostic_json: Mapped[str] = mapped_column(Text, nullable=False, default="{}")
    started_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    finished_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)

    __table_args__ = (
        Index("ix_health_source_runs_snapshot", "snapshot_id"),
        Index("ix_health_source_runs_analyzer", "analyzer_id", "source"),
    )


class HealthRawFact(Base):
    """Immutable source fact; rescore never updates or deletes this row."""

    __tablename__ = "health_raw_facts"
    ...
    source_run_id: Mapped[str | None] = mapped_column(
        String(32), ForeignKey("health_source_runs.id", ondelete="RESTRICT"), nullable=True
    )
    source: Mapped[str] = mapped_column(String(128), nullable=False)
    source_commit: Mapped[str | None] = mapped_column(String(64), nullable=True)
    tool_version: Mapped[str | None] = mapped_column(String(128), nullable=True)
    event_type: Mapped[str] = mapped_column(String(64), nullable=False)
    stable_event_id: Mapped[str] = mapped_column(String(255), nullable=False)
    collected_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    updated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    payload_json: Mapped[str] = mapped_column(Text, nullable=False, default="{}")
    payload_ref: Mapped[str | None] = mapped_column(Text, nullable=True)
    payload_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    redaction: Mapped[str] = mapped_column(String(16), nullable=False, default="partial")
    window_start: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    window_end: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)

    __table_args__ = (
        UniqueConstraint("snapshot_id", "source", "stable_event_id", "source_commit", name="uq_health_raw_fact_event"),
        Index("ix_health_raw_facts_snapshot_type", "snapshot_id", "event_type"),
        Index("ix_health_raw_facts_source_event", "source", "stable_event_id"),
        Index("ix_health_raw_facts_payload_hash", "payload_hash"),
    )


class HealthNormalizedFact(Base):
    """Derived fact with identity/time normalization provenance."""

    __tablename__ = "health_normalized_facts"

    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=_new_uuid)
    snapshot_id: Mapped[str] = mapped_column(
        String(32), ForeignKey("repository_health_snapshots.id", ondelete="RESTRICT"), nullable=False
    )
    raw_fact_id: Mapped[str | None] = mapped_column(
        String(32), ForeignKey("health_raw_facts.id", ondelete="RESTRICT"), nullable=True
    )
    fact_type: Mapped[str] = mapped_column(String(64), nullable=False)
    stable_event_id: Mapped[str] = mapped_column(String(255), nullable=False)
    canonical_contributor_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    normalized_ts: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    original_offset_minutes: Mapped[int | None] = mapped_column(Integer, nullable=True)
    timestamp_corrected: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    identity_confidence: Mapped[float | None] = mapped_column(Float, nullable=True)
    normalization_policy: Mapped[str] = mapped_column(String(64), nullable=False, default="")
    payload_json: Mapped[str] = mapped_column(Text, nullable=False, default="{}")
    source_event_ref: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now_utc)

    __table_args__ = (
        UniqueConstraint("snapshot_id", "stable_event_id", "normalization_policy", name="uq_health_normalized_fact"),
        Index("ix_health_normalized_facts_snapshot_contributor", "snapshot_id", "canonical_contributor_id"),
        Index("ix_health_normalized_facts_snapshot_ts", "snapshot_id", "normalized_ts"),
    )


class HealthMetricValue(Base):
    """Canonical analyzer metric, separate from actionable findings."""
    ...
    __table_args__ = (
        UniqueConstraint("snapshot_id", "analyzer_id", "name", name="uq_health_metric_value"),
        Index("ix_health_metric_values_snapshot_analyzer", "snapshot_id", "analyzer_id"),
        Index("ix_health_metric_values_name", "name"),
    )


class HealthFindingEvidence(Base):
    """Traceable evidence for a finding/finding-like source observation."""

    __tablename__ = "health_finding_evidence"
    ...
    __table_args__ = (
        Index("ix_health_finding_evidence_snapshot_finding", "snapshot_id", "finding_id"),
        Index("ix_health_finding_evidence_location", "path", "line_start"),
    )


class HealthAggregate(Base):
    """Dimension aggregate; score config is explicit for safe rescore."""

    __tablename__ = "health_aggregates"
    ...
    __table_args__ = (
        UniqueConstraint("snapshot_id", "score_config_digest", "scope", "dimension", "name", name="uq_health_aggregate_config"),
        Index("ix_health_aggregates_snapshot_dimension", "snapshot_id", "dimension"),
    )


class HealthRecommendation(Base):
    """Stable remediation identity with criticality-only priority influence."""
    ...
    dimension: Mapped[str] = mapped_column(String(64), nullable=False)
    finding_status: Mapped[str] = mapped_column(String(24), nullable=False, default="open")
    ...
    lifecycle: Mapped[str] = mapped_column(String(16), nullable=False, default="new")
    benefit: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    confidence: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    criticality: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    effort: Mapped[float] = mapped_column(Float, nullable=False, default=1.0)
    risk: Mapped[float] = mapped_column(Float, nullable=False, default=1.0)
    blast_radius: Mapped[float] = mapped_column(Float, nullable=False, default=1.0)
    raw_impact: Mapped[float | None] = mapped_column(Float, nullable=True)
    applied_impact: Mapped[float | None] = mapped_column(Float, nullable=True)
    evidence_json: Mapped[str] = mapped_column(Text, nullable=False, default="[]")
    ...
    __table_args__ = (
        UniqueConstraint("snapshot_id", "recommendation_id", name="uq_health_recommendation_snapshot"),
        CheckConstraint("lifecycle IN ('new', 'open', 'accepted', 'dismissed', 'resolved', 'stale')", name="ck_health_recommendation_lifecycle"),
        Index("ix_health_recommendations_snapshot_priority", "snapshot_id", "priority"),
        Index("ix_health_recommendations_subject", "subject"),
        Index("ix_health_recommendations_finding", "finding_id"),
    )
