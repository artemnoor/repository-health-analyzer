...
def _render_composition_line(result: Any) -> None:
    """Show the canonical result status and evidence coverage in table mode."""
    status = getattr(getattr(result, "status", None), "value", getattr(result, "status", "unknown"))
    evidence = len(getattr(result, "evidence", ()) or ())
    limitations = len(getattr(result, "limitations", ()) or ())
    console.print(
