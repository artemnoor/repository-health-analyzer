...
### Repository Health Analyzer quickstart

This workspace also contains the pinned, direct-copy health composition and its
native adapters. From the repository root:
...
The sample is written to `artifacts/health-sample.json`. The canonical report is
available through CLI/API/MCP with evidence coverage, explicit limitations and
criticality kept separate from health. See
[the architecture](docs/architecture/repository-health.md),
[the analyzer reference](docs/reference/HEALTH_ANALYZER.md) and
[the native tool map](docs/reference/NATIVE_TOOLS.md).

The public ranking is available at `/ranking` after the web app starts. It reads
the public-safe materialized projection, keeps score-band/language/dimension
filters shareable in the URL, and supports comparison of up to four eligible
repositories. Browser verification uses `npm run test:e2e --workspace repowise-web`.
