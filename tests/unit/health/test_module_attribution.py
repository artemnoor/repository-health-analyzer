...
def test_every_analyzer_call_site_supplies_a_repo_root():
    ...
    # The CLI now calls the shared composition adapter; the direct
    # HealthAnalyzer construction therefore lives in that adapter rather than
    # in the command module. Keep the invariant on the actual call sites.
    for module in (full_index, incremental, rescore, composed_health):
