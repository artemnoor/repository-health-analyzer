...
class _SqlAnalysisMixin(AnalysisIndexStore):
    ...
    async def save_health_envelope(
        ...
        *,
        ...
        """Persist the canonical raw/normalized/derived health batch."""
        return await crud.save_health_envelope(
            self._session,
            repository_id,
            context,
            results,
    ...
    async def load_health_snapshot_envelope(self, snapshot_id: str) -> dict[str, Any] | None:
        return await crud.load_health_snapshot_envelope(self._session, snapshot_id)

    async def compare_health_snapshots(
        self, current_snapshot_id: str, previous_snapshot_id: str | None = None
    ) -> dict[str, Any]:
        return await crud.compare_health_snapshots(
            self._session, current_snapshot_id, previous_snapshot_id
        )
    ...
    async def rescore_health_snapshot(
        ...
        *,
        ...
        return await crud.rescore_health_snapshot(
            self._session,
            snapshot_id,
            score_config,
            score_config_digest=score_config_digest,
